...
$p['id']=sqlsav(self::$db,array($p['ref'],$p['voc'],$p['lang']));
return self::edit($p);}
...
foreach(lngs() as $v)if($v!=$r['lang']){
	...
	else $ret.=aj('popup|'.self::$ad.',addfrom|lang='.$v.',ref='.$r['ref'].',from='.$r['lang'].',fvoc='.jurl($r['txt']),$v,'btsav');}
...
if(auth(6)){
	...
	$ret.=aj('admcnn|'.self::$ad.',create',langp('add language'),'btn');}
return div($ret);}
