...
return div(desk::load('desktop','defs',before($p['mdfdr'],'/')),'board');}

static function modifdir($p){$sz=val($p,'sz',8);
//$j=ajs('div,page,2|desktop,savemdfdr','dir='.$p['dir'],'mdfdr');
$j='div,page,2|desktop,savemdfdr|dir='.$p['dir'].'|mdfdr';
$prm=['type'=>'text','id'=>'mdfdr','value'=>$p['dir'],'size'=>8,'onblur'=>'ajbt(this)','onkeypress'=>'checkj(event,this)','data-j'=>$j];
$ret=tag('input',$prm,'',1);
return $ret;}
//rename
static function savemdfbt($p){
if(auth(6) && $p['id'])sqlup(self::$db,'bt',$p['mdfbt'],$p['id']);
return div(desk::load('desktop','defs',$p['dir']),'board');}

static function modifbt($p){
$r=sql('bt,dir',self::$db,'ra','where id="'.$p['id'].'"');
//$j=ajs('div,page,2|desktop,savemdfbt','id='.$p['id'].',dir='.$p['dir'],'mdfbt');
$j='div,page,2|desktop,savemdfbt|id='.$p['id'].',dir='.$p['dir'].'|mdfbt';
$prm=['type'=>'text','id'=>'mdfbt','value'=>$r['bt'],'size'=>8,'onblur'=>'ajbt(this)','onkeypress'=>'checkj(event,this)','data-j'=>$j];
$ret=tag('input',$prm,'',1);
return $ret;}
//del
...
if(isset($p['addrow'])){$r=sqlcols(self::$db,2);
	...
	$nid=sqlsav(self::$db,$rb);}
if(auth(2))$ret=aj('dskmg|desktop,manage|dir='.$dir.',addrow=1',langp('add'),'btn');
...
$r=sql($keys,self::$db,'','where (uid="'.ses('uid').'" or auth<="'.ses('auth').'") '.$wh.' order by id');
foreach($r as $k=>$v){
...
$w.=' or auth<2';//or auth<='.ses('auth').''
return sql($keys,self::$db,'id',$w.' order by dir');}

static function picker($p){$css='bicon'; $usid=ses('uid'); $id=val($p,'id'); $ret='';
$r=sql('id,dir,type,com,picto,bt,auth','desktop','id','where uid="'.$usid.'" and dir like "/documents/img%" order by id desc');// limit 100//and auth<="'.ses('auth').'" 
...
//self::install();
$dir=val($p,'dir'); if($dir && substr($dir,0,1)!='/')$dir='/'.$dir;
$ret=desk::load('desktop','defs',$dir);
if(val($p,'dir') && !$ret)$ret=desk::load('desktop','defs','');
return div($ret,'board');}
}