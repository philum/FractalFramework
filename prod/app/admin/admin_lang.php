...
foreach(lngs() as $v)if($v!=$r['lang']){
	...
	if($id)$ret.=aj('popup|admin_lang,edit|id='.$id,$v,'btn');
	else $ret.=aj('popup|admin_lang,addfrom|app='.$r['app'].',lang='.$v.',ref='.$r['ref'].',from='.$r['lang'].',fvoc='.$r['voc'],$v,'btsav');}
return $ret;}
...
foreach($r as $k=>$v){
	if(auth(6))$ref=aj('popup|admin_lang,edit|id='.$v[0],$v[1],'btn');
	else $ref=$v[1];
	if($v[2])$rb[$k]=[$ref,$v[2]];
	else $rc[$k]=[$ref,$v[2]];}
