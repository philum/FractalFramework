...
static $typs=['var'];
static $conn=0;
static $db2='check_cases';
static $db3='check_valid';
static $open=0;
static $qb='';
...
static function check($p){$bid=val($p,'bid');
...
if($rc)foreach($rc as $k=>$v)if($k){//$re[$k][]=$k;
	foreach($ra as $ka=>$va){
		if(isset($v[$ka])){$c='active'; $bt=ico('check');} else{$c='disactive'; $bt=ico('close');}
		$re[$k][]=aj(self::$cb.$id.'|check,check|bid='.$ka.',id='.$id,$bt,'minicon '.$c);}
	if($re[$k])array_unshift($re[$k],self::usrbt(['uid'=>$k]));} //pr($re);
$ret=div($r['tit'],'tit');
$ret.=mktable($re);
if(!$ex)$ret.=aj(self::$cb.$id.'|check,register|id='.$id,langp('register'),'btsav');
return $ret;}
