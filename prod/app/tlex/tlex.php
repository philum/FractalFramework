...
$ret.=aj('tlxbck|tlex,pub|usr='.$usr,langph('other accounts'),$c);//pub
...
//pane
static function panehead($v){$id=$v['id']; $idv=$v['idv']; $usr=$v['name'];
$avatar=bubble('tlex,profile|usr='.$usr.',small=1',profile::avatarsmall($v),'btxt',1);
$ret=div($avatar,'cell');
$usrnm=lk('/u/'.$usr,$v['pname'],'btxt" title="@'.$usr);
//$time=self::relativetime($v['now']);
$time=span(date('d-m-Y',$v['now']),'small');
$url=lk('/'.$id,$time,'grey');//.ico('external-link',12)
$ret.=div($usrnm.br().$url,'cell');
//$date=pagup('tlex,read|popwidth=600,th='.$id,$time,'grey');
if($v['ib']){
	$to=sqljoin('name',self::$db,'login','uid','v','where '.self::$db.'.id='.$v['ib']);
	$ret.=div(pagup('tlex,read|popwidth=600,th='.$id,lang('in-reply to',1).' '.$to,'grey small'),'cell');}
if($nb=sql('count(id)',self::$db,'v','where ib='.$id))
	$ret.=div(pagup('tlex,read|popwidth=600,th='.$id,$nb.' '.lang($nb>1?'replies':'reply',1),'grey small'),'cell');
$ico=$v['icon']?ico($v['icon']):'';
if($v['ref'])$label=span(span($ico.lang($v['ref']),'tx'),'label'); else $label='';
return div($label.div($ret,'table'),'username');}

static function panefoot($v){
$id=$v['id']; $idv=$v['idv']; $pr='pn'.$idv; $usr=$v['name']; $lg=$v['lg']; $ret='';
...
if(self::$objects)$ret.=toggle($pr.'|tlxcall,keep|idv='.$idv.',id='.$id,icit('download','keep'));
...
static function pane($v,$current=''){
$id=$v['id']; $usr=$v['name']; $v['idv']='tlx'.$id; self::$id=$id;
$head=self::panehead($v);
...
$ret=$head.div($msg.$foot,'bloc_content');//div($avatar,'bloc_left').//
$ret.=div('','','pn'.$v['idv']);
...
$pub=div(self::editor($p),'pblshcnt','pblshcnt'); $stream=div(self::read($p),'','tlxbck');
return $stream;}
